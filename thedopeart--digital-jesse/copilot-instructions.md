...
> Personal portfolio website for Jesse Johnson, a digital entrepreneur and creative professional based in Seattle, WA. The site showcases skills, work examples, and services to attract employment and freelance opportunities.

# Project: DigitalJesse.com
...
> Auto-updated every 10 tool uses and at session end. Check `.claude/sessions/` for full session logs.
...
**Key decisions made:**
- (important decisions so they don't get re-discussed)
...
| Role | Company | Years | Key Achievement |
|------|---------|-------|-----------------|
...
**Quality Sewing** (qualitysewing.com)
- E-commerce Manager
- $4M+ annual revenue, 5,000+ products managed
- Grew organic traffic from 7K to 27K monthly (~300% increase)
- Shifted strategy from paid ads to SEO-driven sales, dramatically reducing ad spend
- Improved conversion rate through on-page optimizations and UI/UX improvements
- Streamlined internal processes leading to higher revenue
- Build custom Shopify apps and 10+ interactive calculators
- YouTube SEO strategy and optimization
- Multi-channel selling: Amazon, Walmart, TikTok Shop
- 6 years total Shopify experience

### Personal Brands & Projects
...
| Client | Work Done |
|--------|-----------|
| Rocky Mountain Steel | Full branding package: company profile, infographics, marketing materials, case studies |
| American Nutriceuticals | SEO blogging, keyword research, graphic design, 3D product mockups, health/wellness content strategy |

---

## Global Style & Tone

All content for this project must follow these rules:
...
- Ahrefs, UberSuggest, E-Rank, Screaming Frog
- Keyword research and content strategy
- Blog writing and copywriting
- YouTube SEO
- Health/wellness content (American Nutriceuticals)

### Design & Creative
- Adobe Photoshop, Illustrator, Premiere Pro, InDesign
- Canva, Figma
- Infographics, pitch decks, wireframes
- Product mockups and branding
- 2D/3D modeling
- Company profiles and case studies (Rocky Mountain Steel)
- UX/UI design

### E-Commerce & Marketing
- Shopify Expert (6 years experience)
- Etsy marketplace management (80K+ views, $53K+ revenue)
- Multi-channel selling (Amazon, Walmart, TikTok Shop)
- Custom Shopify app development
- Meta Ads, Google Ads, paid campaign management
- Email marketing
...
When writing for this site, prioritize:

1. **Specific results over vague claims** - "$2M in sales" beats "successful entrepreneur"
2. **Show don't tell** - Portfolio examples over skill lists
3. **What you can do for them** - Frame experience as value for employers/clients
4. **Proof points** - Brands built, traffic grown, revenue generated
...
/writing-guidelines.md        # Detailed writing standards
/project-checklist.md         # Website rebuild task tracker
/content-structure.md         # Site architecture and page outlines

/assets/                      # Images, logos, portfolio screenshots
/content/                     # Page copy drafts
```
...
Based on review of digitaljesse.com:

1. **Copy is too generic** - "Digital Dreamweaver", "tapestry", "transformative growth" are AI red flags
2. **Missing specific results** - Need concrete numbers and outcomes
3. **Broken pages** - /services returns 404
4. **Structure unclear** - Services vs Portfolio overlap
5. **No clear CTA** - What action should visitors take?
6. **Weak positioning** - Site reads as freelancer, not job candidate

---

## Blog Post Styling Guidelines

### Page Layout
- Outer container: `max-w-7xl` (80rem/1280px), `px-6`, `py-8` (matches navbar width)
- Grid: `lg:grid-cols-[1fr_300px] lg:gap-10` (article + sidebar)
- Sidebar: 300px wide, sticky `top-8`, hidden on mobile (`hidden lg:block`)
- Mobile TOC: collapsible dropdown shown only below `lg:` breakpoint

### Hero Section
- Background: `bg-[#1e1e1e]`, padding `pt-2 pb-6`
- Title: `text-2xl md:text-3xl lg:text-4xl`, gradient text for `**bold**` parts
- Meta row (category, date, read time): `mt-3`, `text-sm text-gray-400`

### Typography
- Headings: `text-2xl font-bold text-gray-900 mb-4`
- Paragraphs: `text-gray-700 leading-relaxed mb-5`
- Bullets: `space-y-2.5`, gold bullet `text-[#b8860b]`
- Links: `text-[#16a34a]` green, `hover:text-[#15803d]`
- Bold: `font-semibold text-gray-900`

### Section Spacing
- Between sections: `mt-8`
- Section dividers: `pb-8 border-b border-gray-100` (all except last)
- Bottom sections (Keep Reading, FAQs, Related): `mt-10`
- Scroll offset: `scroll-mt-8` on h2 elements

### Image Rules
- **Full-width images** (default): `mt-6`, rounded-xl, border, shadow-sm (no bottom margin, section padding handles it)
- **Image-above layout**: `mb-4`, tighter gap between image and heading
- **Two-column images** (image-right, image-left): grid `grid-cols-2` (50/50 split), image is `sticky top-8 self-start`
- **Sidebar images must be simple**: Bold stat, icon, or diagram readable at 280px. No detailed charts or small text. Use 400x300 viewBox.
- **Full-width images**: Use 800x500 viewBox for detailed charts, comparisons, and data visualizations.
- All images stored as SVG source + WebP for production
- WebP conversion: `sharp(svg, {density:300}).resize(1600,1000).webp({quality:90})`
- Sidebar WebP: `sharp(svg, {density:300}).resize(800,600).webp({quality:90})`
- References in `lib/blog-posts.ts` always use `.webp` extension

### Text Contrast — MANDATORY
- **NEVER use light grey text on white or near-white backgrounds.** This is the #1 recurring readability problem.
- Minimum text colors on white/light backgrounds:
  - Primary text: `#0f1a14` or `#1a1a1a` (near-black)
  - Secondary text: `#374151` (dark grey, equivalent to Tailwind `gray-700`)
  - Muted/tertiary text: `#6B7280` (medium grey, equivalent to Tailwind `gray-500`). This is the LIGHTEST acceptable body text.
  - Disabled/placeholder text: `#9CA3AF` (Tailwind `gray-400`). Only for truly inactive or placeholder content, never for readable body copy.
- **Banned on light backgrounds:** `#94A3B8`, `#CBD5E1`, `#E2E8F0`, `#F1F5F9` and any color lighter than `#9CA3AF` for text.
- Icons that convey meaning (checkmarks, X marks, status indicators) must use full-saturation colors (e.g. `#DC2626` red, `#16a34a` green), never washed-out greys.
- When in doubt, pick a darker shade. Err toward too dark, not too light.

### SVG Design Standards
- **Transparent background** (no bg rect) for images on white `bg-white` page
- **Dark-card-on-white** pattern OK for UI mockup illustrations
- Colors: `#1a1a1a` dark text, `#666666` secondary, `#999999` tertiary
- Accent colors: `#b8860b`/`#d4a847` gold, `#22c55e` green, `#3b82f6` blue, `#8b5cf6` purple, `#e74c3c` red
- Font: `system-ui, -apple-system, sans-serif`
- No low-opacity fills for text or important elements
- Test readability at target display size before committing

### CTA Components
- **Inline CTA**: `mt-6`, dark background `bg-[#1e1e1e]`, gold border `border-[#d4a847]/30`, centered text, prominent button
- **Standalone CTA**: `mt-10`, dark background `bg-[#1e1e1e]`, centered, gold border
- **Image + CTA rule**: When a section has both an image and inlineCTA (default layout), image renders above text so the CTA isn't buried under a stacked image

### Sidebar Components
- Unified single card: `bg-[#fafaf9] rounded-2xl border border-gray-200`, sections separated by `divide-y divide-gray-200`
- Section headers: `text-[11px] font-semibold uppercase tracking-wider`, colored per section (gold for TOC, green for related, gray for topics)
- TOC links: `text-[13px]`, active state uses gold `bg-[#b8860b]/8`
- Related posts: `text-[13px]`, hover green `text-[#16a34a]`
- Category pills: `text-[11px] rounded-full`, active gets category color class

### File Naming
- Blog images: `{post-slug}-{number}.svg` / `.webp` for full-width
- Sidebar images: `{post-slug}-sidebar.svg` / `.webp` for two-column
- All files in `public/images/blog/`

---

## Target Audience

1. **Hiring managers** - Looking for digital marketing, e-commerce, or creative roles
2. **Potential clients** - Small businesses needing digital services
3. **Collaborators** - Other creatives or entrepreneurs for projects

Write for audience #1 primarily. The site should function as a living resume that proves capabilities through work examples.

---
> Source: [thedopeart/digital-jesse](https://github.com/thedopeart/digital-jesse) — distributed by [TomeVault](https://tomevault.io).
