...
2. **Authority Boundary (CLAIM):** Gemini cannot issue a `999 SEAL` or self-authorize `arifos_forge`. It must submit all candidate actions to the **L4 ASI (Judge)**.
...
3. **CRP v1.0 Compliance:** If the **L4 ASI (Judge)** emits a `VOID` or `SABAR` verdict, Gemini must stop execution and await Sovereign instruction.

---

## 4. Conflict Resolution Protocol (CRP v1.0)

Gemini follows the **Unidirectional Authority Chain**:
...
---
> Source: [ariffazil/arifos](https://github.com/ariffazil/arifos) — distributed by [TomeVault](https://tomevault.io).
