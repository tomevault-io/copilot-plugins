...
- Round 1 is the exhaustive external inventory across facts, design principles, and behaviors.
  ...
  Before predecessor debt can close, deterministically extend the fresh governing finding with
  every current-occurrence anchor independently authored in its matching violated class outcome,
  preserving authored order and recording the extension in staged audit data. Apply the same
  projection to a non-debt-bound correction occurrence before deriving its violated outcome from
  `classification.assessment_evidence`. Resolve every projected anchor normally; never normalize
  an unresolvable, out-of-bounds, or snapshot-invalid citation into validity.
...
For arbitration decision-evidence admission, validate declared repository references before accepting a decider Cast, using the existing exact snapshot resolver and the existing single format/evidence correction allowance. Preserve NONE and final independent substantiation. Never repair absolute paths by rewriting them, silently drop malformed declared references, add an execution retry, or weaken carried-evidence grounding. Operational revision, tree, symlink-target and blob failures must propagate from the shared resolver without caching empty evidence or spending a declaration retry; retain ordinary invalid-revision, missing-path and non-blob outcomes. Classify absent-object gitlinks from successful literal snapshot tree metadata, never by fetching or following their target. The original failed native batching experiment remains historical; requalification must meet its separately frozen acceptance contract.

For the new snapshot batching requalification, docs/snapshot-batching-requalification-plan.md is the bounded acceptance authority. Preserve the original failed batching campaign as historical evidence. Keep this card's implementation convergence on CODE after PLAN acceptance and retain report attempts before replay can overwrite a default report file.
...
---
> Source: [subvertnormality/paranoia](https://github.com/subvertnormality/paranoia) — distributed by [TomeVault](https://tomevault.io).
<!-- tomevault:4.0:copilot_instructions:2026-09-22 -->
