...
- **Stack:**  Tailwind CSS v3.4.17, GSAP 3 (with ScrollTrigger plugin), Lucide React for icons, no build system - it must be staticly served html js and css.
- **Fonts:** Load via Google Fonts `<link>` tags in `index.html` based on the selected preset.
- **Images:** Use real Unsplash URLs. Select images matching the preset's `imageMood`. Never use placeholder URLs.
- **File structure:**  Single `index.css` for Tailwind directives + noise overlay + custom utilities.
